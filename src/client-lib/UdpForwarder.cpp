	...
	struct UdpPair {
		...
		void trySendRequests() {
			while (!pendingRequests.empty()) {
				auto sent = send(remote.Get(), &pendingRequests[0][0], pendingRequests[0].size(), 0);
				if (sent <= 0) {
		...
		bool tryReadReply(UdpReply& reply) {
			...
			if (available > 0) {
				...
				auto read = recv(remote.Get(), &reply.data[0], reply.data.size(), 0);
				if (read >= 0) {
		...
		void OnLocalSocketSignaled() {
			...
			for (auto& entry : _entries) {
				...
				if ((events.lNetworkEvents & FD_READ) == FD_READ) {
					...
					auto readSize = recvfrom(entry->localSocket.Get(), &req[0], req.size(), 0, (sockaddr*)&clientAddr, &clientAddrLength);
					if (readSize >= 0) {
						req.resize(readSize);
				...
				while (!entry->pendingReplies.empty()) {
					auto sent = sendto(entry->localSocket.Get(), &entry->pendingReplies[0].data[0], entry->pendingReplies[0].data.size(), 0,
	...
	public:
		void Loop() {
			...
			while (_running) {
				...
				auto waitResult = WaitForMultipleObjects(2, events, FALSE, duration_cast<milliseconds>(ClientTimeout).count());
				if (!_running) {
					return;
				}
		...
		void AddEntry(std::uint16_t localPort, std::uint32_t remotePort, const char* remoteAddress) {
			...
			auto localAddress = ResolveUdp("0.0.0.0", localPort);
