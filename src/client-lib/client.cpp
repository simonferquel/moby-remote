...
namespace mobyremote {
	...
	SafeAutoResetEvent MakeAutoResetEvent();
			...
			for (auto& pair : _entries) {
				...
				if ((events.lNetworkEvents & FD_READ) == FD_READ) {
					...
					if (pair.to_remote.size() > 0) {
						auto written = send(pair.remote.Get(), &pair.to_remote[0], pair.to_remote.size(), 0);
						if (written > 0)
				...
				if ((events.lNetworkEvents & FD_WRITE) == FD_WRITE) {
					...
					if (pair.to_local.size() > 0) {
						auto written = send(pair.local.Get(), &pair.to_local[0], pair.to_local.size(), 0);
						if (written > 0)
	...
	public:
		...
		void AddEntry(std::uint16_t localPort, std::uint32_t remotePort, const char* remoteAddress) {
			auto localAddress = Resolve("0.0.0.0", localPort);
